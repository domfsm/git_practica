...
При возникновении конфликта вызываем ```vimdiff``` командой ```git mergetool```;<br>
После сообщения в консоли Hit return to start merge resolution tool (vimdiff), нажимаем Enter, чтобы открыть vimdiff;<br>
```vimdiff``` показывает четыре окна:
* в верхнем левом углу — текущая версия файла в ```HEAD```;
* в правом верхнем углу — версия из ветки ```br2```;
* посередине сверху — версия из ветки, которая является общим<br> предком, то есть из ```main```;
